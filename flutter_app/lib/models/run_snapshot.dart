...
class RunSnapshot {
  ...
  static RunSnapshot? parse(dynamic value) {
    try {
      ...
      if (number(s.data['clock']) < 0 ||
          number(s.data['cooldown']) < 0 ||
          number(s.data['cooldown']) > 320) {
        return null;
      }
      number(s.data['dropX']);
      if (s.data.containsKey('combo') &&
          (s.data['combo'] is! int || (s.data['combo'] as int) < 0)) {
        return null;
      }
      if (s.data.containsKey('lastMerge')) number(s.data['lastMerge']);
      if (s.data.containsKey('watermelon') && s.data['watermelon'] is! bool) {
        return null;
      }
