...
void main() {
  ...
  for (final width in [320.0, 430.0]) {
    testWidgets('screens fit width $width and 130 percent text', (tester) async {
      ...
      for (final screen in [const MenuPage(), const SettingsPage(), const RankingPage(), const GameOverPage(score: 1234, earned: 123)]) {
        await tester.pumpWidget(MultiProvider(providers: [ChangeNotifierProvider.value(value: state), Provider.value(value: sfx)], child: MaterialApp(
          builder: (context, child) => MediaQuery(data: MediaQuery.of(context).copyWith(textScaler: const TextScaler.linear(1.3)), child: child!), home: screen)));
        await tester.pump();
